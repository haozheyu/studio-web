...
async function reloadAssets() {
  assets.value = await listAssets(project.value.id).catch(() => [])
}
...
async function generateScript() {
  const source = project.value.script?.source?.trim()
...
async function scriptToScenes() {
  const script = project.value.script?.generated?.trim()
...
async function sceneToShots(scene: Scene) {
  ...
  try {
    ...
    scene.shots = items.map((item: any) => ({ id: nextShotId(), title: String(item.title || '新镜头'), description: String(item.description || ''), shotSize: String(item.shotSize || '中景'), camera: String(item.camera || '静止'), duration: Math.max(4, Math.min(15, Number(item.duration || 5))), prompt: String(item.prompt || ''), status: 'draft', model: 'flux' as ModelKey }))
...
<template>
  <div class="studio-shell">
    ...
    <main>
      ...
      <section v-else-if="activeNav === '剧本工坊'" class="workspace-page"><section class="scene-head"><div><div class="eyebrow"><span>WRITER</span>DeepSeek-7B</div><h1>小说 / 故事 → 剧本 → Scene</h1><p>原文、AI 剧本和转换后的场次都持久化到当前 Project。</p></div></section><section class="script-workbench"><label>小说 / 故事输入<textarea v-model="project.script!.source" rows="7" placeholder="粘贴小说、故事梗概或剧本原文…"></textarea></label><div class="action-row"><button class="primary" :disabled="sending" @click="generateScript"><Sparkles :size="16" />{{ sending ? '生成中…' : 'DeepSeek 生成剧本' }}</button><button :disabled="!project.script?.generated || converting" @click="scriptToScenes"><Film :size="16" />{{ converting ? '转换中…' : '剧本 → Scene' }}</button><button @click="persistNow"><Save :size="16" /> 保存剧本</button></div><label>生成剧本<textarea v-model="project.script!.generated" rows="16" placeholder="DeepSeek 输出会保存到这里，也可以手工修订。"></textarea></label></section></section>
